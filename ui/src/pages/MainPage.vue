...
const clustcrLogOpen = ref(false);
  ...
  >
    ...
    <PlSlideModal v-model="settingsOpen" close-on-outside-click shadow>
      ...
      <PlAccordionSection :label="strings.titles.advancedSettings">
        <PlNumberField
          v-model="app.model.data.inflation"
          label="MCL inflation"
          :minValue="1.01"
          :step="0.05"
          :maxValue="5.0"
        >
          <template #tooltip>
            Clustering granularity. Lower = coarser (fewer, larger clusters); higher = finer (more,
            smaller). Recommended 1.2–3.0; default 1.2.
          </template>
        </PlNumberField>
