...
# clusTCR runner output (output.csv): seq_id = representative clonotypeKey, cluster = provisional
...
# match the join key). NOT joined here — clusterId is still clusTCR's provisional integer, which
...
if sequence_cols:
    ...
    # clusterId so far is clusTCR's provisional integer. Relabel it to the cluster's MEDOID — the
    ...
    medoid_df = None
    # No sequence columns -> no medoid relabel; clusterId stays clusTCR's integer. Join the label
